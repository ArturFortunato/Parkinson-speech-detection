...
\begin{table}
	...
	\begin{tabular}{lcccccccc}
		...
		{\\\hline\dataset & \solver & \alpha & \iterations & \accuracy  & \precision & \recall & \specificity & \fscore}
	\end{tabular}
	\caption{\label{semi-1}Top five semi language-independent experiment result using architecture 1.} Dataset column legend: \textbf{M} - MDVR\_KCL, \textbf{F} - FralusoPark, \textbf{G} - Gita. First dataset was used in its entirely for training, the second one was partially used for training and partially for testing.
\end{table}
...
\begin{table}
	...
	\begin{tabular}{lcccccccc}
		...
		{\\\hline\dataset & \solver & \alpha & \iterations & \accuracy  & \precision & \recall & \specificity & \fscore}
	\end{tabular}
	\caption{\label{semi-2}Top five semi language-independent experiment result using architecture 2.} Dataset column legend: \textbf{M} - MDVR\_KCL, \textbf{F} - FralusoPark, \textbf{G} - Gita. First dataset was used in its entirely for training, the second one was partially used for training and partially for testing.
\end{table}
